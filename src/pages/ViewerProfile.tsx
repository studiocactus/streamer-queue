...
export default function ViewerProfile() {
  ...
  useEffect(() => {
    ...
    setError(false)
    setViewer(null)
    Promise.resolve(supabase.rpc('get_public_viewer_profile', { p_login: login })).then(({ data, error }) => {
  ...
  if (error) return <div className="page-section"><EmptyState title="Não foi possível carregar o perfil" description="Confira sua conexão e tente novamente." action={<Button onClick={() => setRetry((value) => value + 1)}>Tentar novamente</Button>} /></div>
  if (!viewer) return (
  ...
  const socialLinks = Object.entries(viewer.social_links ?? {}).flatMap(([network, value]) => {
      ...
      <div className="app-shell max-w-4xl space-y-6">
        <section className="overflow-hidden rounded-3xl border border-border bg-bg-secondary">
          ...
          <div className="px-5 pb-6 sm:px-8">
            ...
            <div className="mt-4">
              <h1 className="break-words text-2xl font-bold text-content-primary">{viewer.display_name}</h1>
              ...
              {viewer.bio && <p className="mt-5 max-w-2xl whitespace-pre-line break-words text-sm leading-relaxed text-content-secondary">{viewer.bio}</p>}
        ...
        <section className="rounded-3xl border border-border bg-bg-secondary p-5 sm:p-8">
          ...
          {viewer.moderated_channels.length === 0 ? <p className="mt-6 rounded-2xl border border-border bg-bg-tertiary/50 px-4 py-5 text-sm text-content-muted">Este viewer ainda não modera nenhum canal.</p> : <div className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-2">{viewer.moderated_channels.map((channel) => <Link key={channel.slug} to={streamerPath(channel.slug)} className="flex min-w-0 items-center gap-3 rounded-2xl border border-border bg-bg-tertiary/60 p-3 transition-colors hover:border-brand-purple/35"><Avatar src={channel.avatar_url} alt={channel.channel_name} fallback={channel.channel_name} size="sm" /><span className="min-w-0 flex-1 truncate text-sm font-medium text-content-primary">{channel.channel_name}</span><ExternalLink size={14} className="shrink-0 text-content-muted" /></Link>)}</div>}
        </section>
      </div>
    </div>
  )
